...
extension RecordingManagerTests {
    private func makeLifecycleOperations(_ state: RecordingLifecycleTestState) -> RecordingLifecycleCoordinator.Operations {
    ...
    func testLifecycleCancellationDuringStartupCleansStateAndExclusivity() async {
        ...
        await coordinator.cancel(
            isRecording: false,
            isStarting: true,
            operations: makeLifecycleOperations(state),
        )
    ...
    func testLifecycleCancellationDuringRecordingCleansStateAndFiles() async {
        let state = RecordingLifecycleTestState()
        state.captureActive = true
        state.exclusive = true
    ...
    func testLifecycleStopWithoutTranscriptionCancelsIncrementalBeforeReset() async {
        let state = RecordingLifecycleTestState()
        state.captureActive = true
        state.exclusive = true
        state.incrementalActive = true
        let coordinator = RecordingLifecycleCoordinator()
        let operations = makeLifecycleOperations(state)
    ...
    func testLifecycleStopWithTranscriptionPreservesFullFileHandoff() async {
        let state = RecordingLifecycleTestState()
        state.captureActive = true
        state.exclusive = true
        let coordinator = RecordingLifecycleCoordinator()
    ...
    func testLifecycleFinalizationFailureResetsStateAndReleasesExclusivity() async {
        let state = RecordingLifecycleTestState()
        state.captureActive = true
        state.exclusive = true
        let coordinator = RecordingLifecycleCoordinator()
    ...
    func testUnexpectedRecorderFailureReleasesExclusivityAndClearsCaptureTasks() async {
        let state = RecordingLifecycleTestState()
        state.captureActive = true
        state.exclusive = true
        state.incrementalActive = true
        ...
        XCTAssertFalse(state.incrementalActive)
        XCTAssertFalse(state.temporaryTaskActive)
        XCTAssertEqual(state.events, ["cancelPostStartTasks", "cancelIncremental", "resetState", "endExclusivity"])
    }
}
