...
Generated by the improve skill on 2026-06-25 and extended through 2026-07-12. Execute in the order below unless
...
- At the original 2026-06-25 audit, related open issues were #53, #59, #97, and #98; current issue status is recorded below.

## 2026-07-12 SwiftUI/Swift 6.2 and open-issue follow-up scope

- Effort: standard, focused on the `swiftui-pro` audit, Swift 6.2/concurrency readiness, SwiftUI data flow/accessibility, structural hotspots, Settings search routing, provider setup, storage performance, and all currently open GitHub issues.
- Audited: current `main` at `80ed5788`, Xcode/package language settings, SwiftUI patterns, Observation usage, concurrency patterns, lint/preview/guidance checks, current source sizes, storage/provider/audio paths, all 19 open GitHub issues, and recent implementation history.
- Not audited: runtime screenshot QA, live provider behavior, signed release behavior, production CloudKit behavior, and hardware-only audio profiling beyond current code evidence.
- Reuse decision: extend existing `macos-app-engineering`, `swift-concurrency-expert`, `accessibility-audit`, `KeychainManager`, `AppSettingsStore`, Core Data query contracts, Assistant phase/orchestrator types, and existing test suites. Do not create parallel UI, storage, provider, or recording pipelines.
...
## GitHub issue reconciliation — 2026-07-12

| Issue | Current decision | Priority / plan |
|---|---|---|
| #124 | Keep open; prefix routing remains brittle | P2 / 044 |
| #114 | Keep open; recording runtime still exceeds complexity limits | P1 / 042 |
| #113 | Keep open; rich-text editor family remains oversized | P1 / 043 |
| #112 | Keep open; settings/transcription hotspots remain | P2 / 047 |
| #108 | Keep open; historical finding needs current triage | P3 / 051 |
| #107 | Keep open; fixture-only benchmark is still intentional limitation | P3 / 050 |
| #98 | Keep open; FTS remains measurement-gated | P3 / 045 |
| #97 | Keep open; incremental update strategy remains measurement-gated | P2 / 045 |
| #91 | Keep open; automation is an uncommitted product direction | P3 / 052 |
| #66 | Keep open; Launch at Login failure lacks actionable UI feedback | P2 / 048 |
| #65 | Keep open; custom providers still assume `/models` | P1 / 046 |
| #63 | Keep open; save-without-verification is not available | P1 / 046 |
| #54 | Keep open; CloudKit requires a privacy/product decision | P3 / 053 |
| #51 | Keep open; storage architecture lacks a maintained reference | P2 / 049 |
| #53 | Closed; query/filter slice implemented and split into #97/#98/plan 001 | completed |
| #59 | Closed; buffer pool and lease registry now exist | completed |
| #60 | Closed; original 10-second premise is stale and configurability is not justified | completed |
...
| 038 | Define Swift 6.2 and macOS SwiftUI project standards | P1 | M | - | TODO |
...
- 038 must run before 039-041 because it defines the canonical Swift/macOS/SwiftUI policy and repairs skill governance.
- 039 must run before 040, 042, 046, 047, 050, and 051 because those plans depend on explicit Swift 6.2 actor/concurrency rules.
- 040 should migrate one stable UI boundary at a time; it must not become a repository-wide mechanical conversion.
- 041 should run before 043 because the editor decomposition must preserve the project's current accessibility and interaction standard.
- 042 addresses the remaining recording runtime concentration after plan 034 already reduced the indicator file; it must not repeat completed indicator work.
...
- Close issue #53 after recording that its original query/filter slice is implemented; retain #97/#98 as separate measurement-gated follow-ups.
- Close issue #59 after recording that `AudioPCMBufferPool` and `AudioPCMBufferLeaseRegistry` now exist in `SystemAudioRecorder`.
- Close issue #117 after recording that Assistant recording/transcription/AI/normalization/dispatch responsibilities are already split into phase/orchestrator types with focused tests.
